...
template <int K_SPLITS_, int COL_SPLITS_>
...
{
  constexpr int K_SPLITS = K_SPLITS_;
  constexpr int COL_SPLITS = COL_SPLITS_;
  constexpr int DC_PER_MERGE_THREAD = DC / (MERGE_THREADS * COL_SPLITS);
  static_assert(DC % (MERGE_THREADS * COL_SPLITS) == 0, "merge dc coverage");
  const int t   = blockIdx.x;
  const int h   = blockIdx.y;
  const int cta_c = blockIdx.z;
  const int tid = threadIdx.x;
  ...
  const int d_base = cta_c * (DC / COL_SPLITS) + tid * DC_PER_MERGE_THREAD;
...
template <int K_SPLITS_, int BLOCK_N_, int COL_SPLITS_, int BIT>
    ...
    __nv_bfloat16* out, float* lse_out)
{
  ...
  dim3 grid_merge(Nt, H, COL_SPLITS_);
  attn_merge_kernel<K_SPLITS_, COL_SPLITS_><<<grid_merge, MERGE_THREADS, 0, stream>>>(
